...
This API and vertex-web (the Next.js frontend) are deployed on different domains (AWS Lambda behind CloudFront at api.samuelsantana.dev, vs. Vercel — Render was retired on 31/08/2026). A `Set-Cookie` issued directly from an OAuth callback response here would be scoped to *this API's own domain* — vertex-web's `cookies()` calls could never see it, regardless of polling. `handleOAuthCallback` in `auth.controller.ts` does not set a cookie at all:

1. It mints a random, single-use exchange code via `AuthService.createOAuthExchangeCode` (60s TTL, stored in the `oauth_exchange_codes` table). It used to live in an in-memory `Map`, which was correct only for a single-instance deployment: the request that mints a code and the `POST /auth/exchange` that spends it are two separate HTTP calls, and anything running more than one process — a second instance, a rolling deploy, a Lambda execution environment — can land them in different memory. The row stores a SHA-256 of the code and the user's id, never the code and never a frozen token payload, so the token is rebuilt from the current row when the code is spent.
...
`trustProxy: true` is set on the Fastify adapter, but **the limiter deliberately does not count against `request.ip`**. Behind CloudFront that is the leftmost `X-Forwarded-For` entry, and CloudFront appends to the header rather than replacing it — so it is a value the caller picks. Counters are keyed on `CloudFront-Viewer-Address`, and only for requests that proved they came through the CDN via the `x-origin-verify` header. See `src/common/throttler/client-tracker.ts` and `src/common/edge/edge-origin.guard.ts`.
