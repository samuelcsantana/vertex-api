...
## 🔒 Access Control
Every `POST`/`PATCH`/`DELETE` across posts, topics, about-page content, and uploads requires `JwtAuthGuard` + `AdminGuard` (`role !== 'admin'` → `ForbiddenException`). Comments are the deliberate exception: any authenticated user can create one (`JwtAuthGuard` alone), but deleting one still checks `isOwner || isAdmin` in `CommentsService.remove` — never just "is logged in." If you add a new mutation endpoint, match whichever of these two patterns actually fits (admin-only vs. owner-or-admin) rather than defaulting to bare `JwtAuthGuard`.
